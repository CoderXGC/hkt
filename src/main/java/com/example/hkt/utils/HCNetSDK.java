...
public interface HCNetSDK extends StdCallLibrary {
    ...
    HCNetSDK INSTANCE = (HCNetSDK) Native.loadLibrary(ApplicationInfo.hikvisionDllPath + "HCNetSDK.dll", HCNetSDK.class);
    /***宏定义***/
    //常量
...
//网络(NET_DVR_NETCFG_V30结构)
    ...
    public static final int COMM_UPLOAD_HEATMAP_RESULT_PDC = 0x4018;//热度图按人数统计数据上传事件
    public static final int COMM_UPLOAD_HEATMAP_RESULT_DURATION = 0x4019;//热度图按人员停留时间统计数据上传事件
    public static final int COMM_UPLOAD_HEATMAP_RESULT_INTERSECTION = 0x4020; //路口分析热度值结果上传
    public static final int COMM_FIREDETECTION_ALARM = 0x4991; //火点检测报警
    /*************操作异常类型(消息方式, 回调方式(保留))****************/
...
//校时结构参数
    ...
    public static class NET_DVR_SINGLE_HEATMAP_RESULT_PDC extends Structure {
        public int dwMaxHeatMapValue;//最大热度值
        public int dwMinHeatMapValue;//最小热度值
        public int dwTimeHeatMapValue;// (时间热度值)平均热度值
        public short wArrayLine;//图片像素点行值
        public short wArrayColumn;//图片像素点列值 （当行列值为0的时候，像素点值内存信息不存在）
        public Pointer pBuffer;  //热度图片像素点数据信息
        public byte[] byRes = new byte[32];
    }


    //热度图报警上传
    public static class NET_DVR_HEATMAP_RESULT_PDC extends Structure {
        public int dwSize;
    ...
    */
        public NET_DVR_SINGLE_HEATMAP_RESULT_PDC[] struSingleHeatMap = new NET_DVR_SINGLE_HEATMAP_RESULT_PDC[2];
        public short wCurNumber;//当前人数
        public short wLeaveNumber;//离开人数
        public byte[] byRes1 = new byte[60];
    }


    //热度图报警上传
    public static class NET_DVR_HEATMAP_RESULT extends Structure {
        public int dwSize;
        public NET_VCA_DEV_INFO struDevInfo;/*设备信息*/
        public NET_DVR_TIME_EX struStartTime;/*开始时间*/
        public NET_DVR_TIME_EX struEndTime;/*结束时间*/
        ...
        public int dwTotalTime;//停留时间总和，单位秒，按人员停留时间报警时上传
...
//NET_DVR_Login()参数结构
    ...
    public static class NET_DVR_PLATE_INFO extends Structure {
        ...
        public byte[] sPlateCategory = new byte[MAX_CATEGORY_LEN/*8*/];//车牌附加信息, 即中东车牌中车牌号码旁边的小字信息，(目前只有中东地区支持)
    ...
    */
    ...
    public static class NET_DVR_TIME_V30 extends Structure {
        ...
        public byte cTimeDifferenceM;            //与UTC的时差（分钟），-30, 30, 45，+表示东区，byISO8601为1时有效
    }
    ...
    public static class NET_ITS_PICTURE_INFO extends Structure {
        ...
        public NET_VCA_RECT struPlateRect;    //车牌位置,当byType为8-非机动车;9-行人时，表示人体坐标
        public NET_VCA_RECT struPlateRecgRect;   //牌识区域坐标，当图片类型为12-主驾驶室图13-副驾驶室图是，该坐标为驾驶员坐标
...
//截止2004年10月5日,共116个接口
    ...
    boolean NET_DVR_CaptureJPEGPicture_WithAppendData(int lUserID, int lChannel, NET_DVR_JPEGPICTURE_WITH_APPENDDATA lpJpegWithAppend);
...
//2007-11-30 V211支持以下接口 //11
    ...
    public static class NET_DVR_MANUALSNAP extends Structure {
        public byte byOSDEnable = 0;    //抓拍图片上是否关闭OSD信息叠加：0- 不关闭(默认)，1- 关闭
        public byte byLaneNo = 1;   //  车道号，取值范围：1~6，默认为1
        public byte[] byRes = new byte[22]; // 保留，置为0
    }
}
